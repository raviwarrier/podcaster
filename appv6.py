...
Always refer to the article and mention it's title at least once. Do not use made-up events.
...
def get_title_from_input(
    ...
    if input_type == "URL" and url_input:
        try:
            ...
            logger.warning(f"Could not fetch title from URL: {e}")
    elif input_type == "Text" and text_input:
        # Extract the first line or H1 heading
        lines = text_input.strip().splitlines()
        if lines:
            title = lines[0].strip()
    elif input_type == "Markdown" and markdown_input:
        # Extract the first H1 heading
        headings = re.findall(r'^# (.*)', markdown_input, re.MULTILINE)
        if headings:
            title = headings[0].strip()
    elif input_type == "PDF" and pdf_files:
        # Try to get the title from PDF metadata
        try:
            reader = PdfReader(pdf_files[0].name)
            if reader.metadata.title:
                title = reader.metadata.title.strip()
        except Exception as e:
            logger.warning(f"Could not fetch title from PDF metadata: {e}")
    # Clean up the title to make it filename-safe
    title = re.sub(r'[^\w\s-]', '', title).strip()
    title = re.sub(r'[\s]+', '_', title)
    return title or "podcast"

def get_audio(text: str, voice: str, audio_model: str, api_key: str = None) -> bytes:
    client = OpenAI(
...
def generate_audio(
    ...
    text_model: str = "o1-mini",
    audio_model: str = "tts-1",
    speaker_1_voice: str = "alloy",
    speaker_2_voice: str = "echo",
    api_base: str = None,
    audio_format: str = "mp3",  # Added audio_format parameter
    file_name: str = "",        # Added file_name parameter
    ...
    with cf.ThreadPoolExecutor() as executor:
        ...
        for line in llm_output.dialogue:
            ...
            future = executor.submit(
                get_audio, line.text, voice, audio_model, openai_api_key
            )
    ...
    if audio_format != "mp3":
        audio = convert_audio_format(audio, "mp3", audio_format)

    # Use a temporary file with the title in the filename and correct extension
    ...
    # Delete any files in the temp directory that end with .mp3 or .ogg and are over a day old
    for file in glob.glob(f"{temporary_directory}*.mp3") + glob.glob(f"{temporary_directory}*.ogg"):
...
def validate_and_generate_audio(*args):
    # Adjust indices based on the new inputs
    input_type = args[0]
...
""") as demo:
    ...
    with gr.Row(elem_id="main_container"):
        # Left Column: Input Fields and Audio Configurations
        with gr.Column(scale=1):
            gr.Markdown("## Input and Audio Configuration")
            input_type = gr.Radio(
                label="Input Type",
                choices=["URL", "Text", "Markdown", "PDF"],
                value="URL",
                info="Select the type of input you are providing."
            )
            url_input = gr.Textbox(label="Enter Webpage URL", lines=1, placeholder="https://example.com", visible=True)
            ...
            pdf_files = gr.Files(label="Upload PDFs", file_types=["pdf"], visible=False)

            # Function to update input components based on selected input type
            ...
            # Bind the function to the input_type change event
            input_type.change(
                fn=update_input_components,
                inputs=[input_type],
                outputs=[url_input, text_input, markdown_input, pdf_files]
            )
            ...
            audio_format = gr.Dropdown(
                ...
                choices=["mp3", "ogg"],
                value="mp3",
                info="Select the output audio format.",
            )
            file_name_input = gr.Textbox(
                label="Output File Name",
                placeholder="Enter desired file name without extension...",
                value="",
                info="Specify the file name for the output audio file.",
            )
            api_base = gr.Textbox(
        ...
        # Right Column: Instruction Templates and Fields
        with gr.Column(scale=1):
            gr.Markdown("## Instruction Templates and Fields")
            template_dropdown = gr.Dropdown(
                label="Instruction Template",
                choices=list(INSTRUCTION_TEMPLATES.keys()),
                value="podcast",
                info="Select the instruction template to use. You can also edit any of the fields for tailored results.",
            )
            ...
            scratch_pad_instructions = gr.Textbox(
                ...
                info="Provide the scratch pad instructions for brainstorming content.",
            )
            prelude_dialog = gr.Textbox(
                label="Prelude Dialog",
                lines=5,
                value=INSTRUCTION_TEMPLATES["podcast"]["prelude"],
                info="Provide the prelude instructions before the dialogue.",
            )
            ...
            # Use Edited Transcript and Feedback
    ...
    # Move the "Generate Audio" button here
    ...
    submit_btn.click(
        ...
        inputs=[
            ...
            audio_format, file_name_input,  # Added new inputs
    ...
    # Bind the regenerate button to the function
    ...
    # Commented out the README content display
